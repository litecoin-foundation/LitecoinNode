...
if [[ $ANSWER =~ ^([yY])$ ]]
then
	wget $DEBIAN_BASE/$DIST-install-litecoin.sh -P $HOME
	...
	rm -f -v $HOME/$DIST-install-litecoin.sh
		...
		then
			...
			then
				...
				wget $DEBIAN_BASE/$DIST-update.sh -P $HOME/scripts
...
if [[ $ANSWER =~ ^([yY])$ ]]
then
	wget $DEBIAN_BASE/$DIST-install-statuspage.sh -P $HOME
...
if [[ $ANSWER =~ ^([yY])$ ]]
then
	wget $DEBIAN_BASE/$DIST-install-createswap.sh -P $HOME
